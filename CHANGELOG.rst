...
----------
  ...
  * ADDED: Template USB Audio application app_usb_aud_template
  * CHANGED: Audio HW configuration in app_usb_aud_xk_316_mc
    and app_usb_aud_xk_evk_xu316 no longer generate the fixed MCLK in board
    support functions. Instead, the fixed MCLK generated by lib_xua (XUA_AudioHub) is used.
  ...
  * Changes to dependencies:
    ...
    - lib_xua: 5.1.0 -> 5.2.0
      ...
      + ADDED:     Support for define ``USER_MAIN_TASKS`` with the same
        functionality as ``USER_MAIN_CORES`` i.e. to allow insertion of tasks
        into the `main()` function (``USER_MAIN_CORES`` to be deprecated in a
        future release)
